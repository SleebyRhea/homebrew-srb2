class Srb2 < Formula
  ...
  option "build-debug", "Set the target build type to Debug"
  option "app-dir", "Set the resting place for the SRB2.app symlink"
...
+	set(CMAKE_FIND_FRAMEWORK LAST)
 ...
 set(CMAKE_RUNTIME_OUTPUT_DIRECTORY "${CMAKE_BINARY_DIR}/bin")'
  ...
  def install
    build_type = "Release" unless build.with? "build-debug"
    build_type = "Debug" if build.with? "build-debug"
    ...
    chdir "#{buildpath}/build" do
      ...
      chdir "#{buildpath}/assets/installer" do
        ...
        Dir.foreach(".") do |f|
          ...
          next if f == "." or f == ".."
          cp_r "#{f}", "#{buildpath}/build/bin/SRB2SDL2.app/Contents/Resources/"
        end
      end
